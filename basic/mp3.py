...
class Mp3Panel(wx.Panel):
    ...
    def update_mp3_listing(self, folder_path):
        ...
        print(f"DEBUG {folder_path + '/*.mp3'}")
        mp3_objects = []
        index = 0

        def fill_na(txt: str) -> str:
        ...
        for mp3 in mp3s:
            ...
            self.list_ctrl.InsertItem(index, fill_na(mp3_object.tag.artist))
            self.list_ctrl.SetItem(index, 1, fill_na(mp3_object.tag.album))
            self.list_ctrl.SetItem(index, 2, fill_na(mp3_object.tag.title))
...
class Mp3Frame(wx.Frame):
    ...
    def create_menu(self):
        ...
        open_folder_menu_item = file_menu.Append(
            wx.ID_ANY, "Open File", "Open a folder with MP3s"
        )
